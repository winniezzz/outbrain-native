 ...
 */
export function buildWidgetHtml(props: OutbrainWidgetProps): string {
  const {
    widgetId,
    widgetIndex = 0,
    articleUrl,
  ...
  if (extId) configParams["extId"] = extId;
  if (extSecondaryId) configParams["extSecondaryId"] = extSecondaryId;
  if (pubImpId) configParams["pubImpId"] = pubImpId;
  if (consentV1) configParams["consentV1"] = consentV1;
  if (consentV2) configParams["consentV2"] = consentV2;
  if (ccpaString) configParams["ccpaString"] = ccpaString;
  if (userId) configParams["userId"] = userId;
  if (lang) configParams["lang"] = lang;
  if (psub) configParams["psub"] = psub;

  // Determine URL type for Platforms API
  let urlType = "articleUrl";
  if (usePortalUrl) urlType = "portalUrl";
  else if (useBundleUrl) urlType = "bundleUrl";
  else if (useContentUrl) urlType = "contentUrl";
